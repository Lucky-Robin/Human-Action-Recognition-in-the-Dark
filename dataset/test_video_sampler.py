...
def video_sample(validation_file_path):  # raw/test
    outdir = 'sampled/test'
        ...
        if frame.isOpened():
            ...
            while success:
                if time % sample_frame_num == 0:
                    outfile = outdir + '/' + sourceFileName[:-4] + '_sampled_' + str(
                        c) + '.jpg'  # sampled/test/0_sampled_'c'.jpg
                    cv2.imwrite(outfile, image)
                    c += 1
                time += 1
                cv2.waitKey(1)
                success, image = frame.read()
        if c != 7:  # 380.mp4 has additional frames, manually delete
...
# Directly and independently run test_video_sampler.py to sample test dataset
if __name__ == '__main__':
    validation_file_path = 'raw/test'
