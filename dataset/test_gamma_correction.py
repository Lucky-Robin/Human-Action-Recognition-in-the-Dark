...
def gamma_intensity_correction(cur_path, gamma):
    ...
    for image_name in os.listdir(cur_path):
        image_path = os.path.join(cur_path, image_name)     # sampled/test/0_sampled_0.jpg
        convert_image = cv2.imread(image_path)
        image_gamma_correct = cv2.LUT(convert_image, LU_table)
        outfile = 'preprocessing/test/' + image_name    # preprocessing/test/0_sampled_0.jpg
        cv2.imwrite(outfile, image_gamma_correct)
        print(image_name, 'Gamma Correction Finished')


# Directly and independently run test_gamma_correction.py to do gamma correction for test dataset
if __name__ == '__main__':
    sampled_path = 'sampled/test/'
